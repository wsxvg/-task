...
def get_city_weather_from_api(city_name, coords):
    ...
    if weather_now and weather_now.get('now'):
        ...
        feels_like = weather_now['now'].get('feelsLike')
...
def create_summary_board(all_weather):
    ...
    for i, city in enumerate(all_weather):
        ...
        # 舒适度指数提醒（仅在没有其他提醒时显示）
        comfort_info = indices.get('comfort')
        if comfort_info and len(alerts) < 2:  # 只在提醒较少时显示舒适度
            comfort_emoji, comfort_desc = get_comfort_index_desc(comfort_info['level'], comfort_info['category'])
            if comfort_info['level'] == 1:  # 舒适
                alerts.append(f"{comfort_emoji} 体感舒适")
            elif comfort_info['level'] >= 4:  # 不舒适
...
def create_comprehensive_advice(all_weather):
    ...
    if rainy_cities:
        ...
        for city in rainy_cities:
            ...
            if rain_time:
                rain_time_cities.append(f"「{city['name']}」{rain_time}")
            else:
                regular_rain_cities.append(city['name'])
        
        # 合并所有降雨城市
        all_rain_info = []
        if rain_time_cities:
            all_rain_info.extend(rain_time_cities)
        if regular_rain_cities:
            all_rain_info.append('、'.join([f"「{city}」" for city in regular_rain_cities]))
        
        if all_rain_info:
            umbrella_emoji = "⛈️" if any("雷" in c['textDay'] for c in all_weather) else "☔️"
            tips_section.append(f"{umbrella_emoji} 降雨提醒：{' '.join(all_rain_info)} 记得带伞哦！")
    ...
    for city in all_weather:
        ...
        if comfort_info and comfort_info['level'] >= 4:  # 很不舒适及以上
            comfort_emoji, comfort_desc = get_comfort_index_desc(comfort_info['level'], comfort_info['category'])
            comfort_issue_cities.append(city['name'])  # 只保存城市名称
